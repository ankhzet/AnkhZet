...
class UASparser
{
		 ...
		 */
		public function __construct($cacheDirectory = null, $updateInterval = 0) {
				if ($cacheDirectory) $this->SetCacheDir($cacheDirectory);
				if ($updateInterval) $this->updateInterval = intval($updateInterval);
		}
		...
		function _loadData() {
				...
				if (file_exists($this->_cache_dir.'/cache.ini')) {
						...
						$lastUpdate = intval($cacheIni['lastupdate']);
						if ($this->updateInterval && ($lastUpdate < time() - $this->updateInterval || $lastUpdate)) {
								$this->_downloadData();
						}
				}
				else {
