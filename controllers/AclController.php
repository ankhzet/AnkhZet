	...
	class ACLController extends AdminViewController {
		...
		function listActions($c) {
			...
			foreach ($c[0] as $code) {
				...
				$name = strtolower($e[1]);
				$exten = strtolower(uri_frag($e, 3, null, 0));

				$_ = array('name' => $name, 'extends' => $exten, 'loaded' => true, 'actions' => array());
				if ($exten) {
					$e = uri_frag($this->classes, $exten, null, 0);
					$_['loaded'] = $e && $_['loaded'];
					if ($_['loaded'])
						$_['actions'] = $e['actions'];
				}
		...
		public function actionMain($r) {
			...
			if (post('actions')) {
				...
				foreach ($this->classes as $ctl => $class) {
					...
					sort($class['actions']);
					$o[isset($m[1]) ? $m[1] : ''] = array('extends' => $class['extends'], 'actions' => $class['actions']);
					unset($this->classes[$ctl]);
				}
				ksort($o);
			...
			case 'modify':
				...
				foreach ($routes as $rid => $route) {
					$r1 = explode('/', $route);
					if ($r2 = $r1[0])
						$rr[$r2] = isset($routing[$r2]) ? $routing[$r2] : $r2;
				...
				foreach ($g as $group => $id) {
					if (!isset($acls[$id]) || (array_search(ACL::ACC_ALL, is_array($acls[$id]) ? $acls[$id] : array($acls[$id])) === false))
						$c->set(array('acl', $group, 'allow'), ACL::ACC_ALL);

					$c->set(array('acl', $group, 'disallow'), isset($acls[$id]) ? $acls[$id] : null);
				}
...
//				debug($rr, 'routing');
			...
			case 'add':
				...
				$_routed = normalize_uri(stripslashes(post('redir')));
				...
				if (array_search($route, $routing) === false) {
				...
				locate_to('/acl');
			case 'del':
				$route = normalize_uri(stripslashes(post('uroute')));
				unset($routing[$route]);
				$c->set('routing', $routing);
				$c->save();
				locate_to('/acl');
			...
			foreach ($acl as $group => $acls) {
				if (isset($acls['disallow']) && is_array($acls['disallow']) && count($acls['disallow']))
					foreach ($acls['disallow'] as $route)
						allow_route($r, $route, $id++, false);
				else
					if (isset($acls['disallow']) && ($route = trim($acls['disallow'])))
						allow_route($r, $route, $id++, false);
			}
			...
			foreach ($routing as $route => $redirect) {
				...
				$rid = uri_frag($routes, $uri, count($routes));
				$routes[$uri] = $rid;
				foreach ($g as $group => $id) {
...
?>
	...
	<?=join(PHP_EOL . "\t", $uroutes)?>
		...
		<form action="/acl" method="post">
			...
			<div><label>Новый маршрут:</label><input type=text name=route value="<?=htmlspecialchars(stripslashes(post('route')))?>" /></div>
			<div><label>Редирект:</label><input type=text name=redir value="<?=htmlspecialchars(stripslashes(post('redir')))?>" /></div>
			<div><label></label><input type=submit value=" Добавить маршрут " /></div>
		</form>
		<form action="/acl" method="post">
			<h3><span></span>Добавление групп пользователей</h3>
			<input type=hidden name=action value=group />
			<div><label>Новая группа пользователей:</label><input type=text name=group value="<?=htmlspecialchars(stripslashes(post('group')))?>" /></div>
