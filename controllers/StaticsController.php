...
';
		...
		function actionAdd($r) {
			...
			if ($_REQUEST[action] == 'add') {
				...
				foreach ($this->EDIT_STRINGS as $key) {
					$v[$key] = addslashes(trim($_REQUEST[$key]));
				...
				if (!count($error)) {
					...
					Loc::$config->set('ru.titles.' . $v[link], stripslashes(stripslashes(str_replace(array('=', PHP_EOL), array('&#61;', ''), $v[title]))));
					Loc::$config->save();
					...
					if (($id = $pa->add($v))) {
						...
						header('Location: /templates/ru/' . $v[link] . '?back=statics');
						die();
					} else
		...
		function actionDelete($r) {
			...
			if ($id) {
					...
					header('Location: /' . $this->_name . '/page/' . intval($_REQUEST[page]));
			} else
